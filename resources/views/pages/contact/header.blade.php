<section class="header-contact">
    @include('utils.menu')
    <h1 class="contact-title">Contact Us</h1>
    <hr class="underline center gradient-galaxy">
    <span class="contact-overtitle">Send us a message</span>
    <div class="banner-contact" data-aos="fade-up">
        <div data-aos="fade-up" data-aos-delay="50" class="banner-single">
            <i class="fa fa-3x fa-phone"></i>
            <h2 class="banner-single-title">Phone</h2>
            <span class="banner-single-text">630-359-5511</span>
        </div>
        <div data-aos="fade-up" data-aos-delay="100" class="banner-single">
            <i class="fa fa-3x fa-map-marker-alt"></i>
            <h2 class="banner-single-title">Address</h2>
            <span class="banner-single-text">630-359-5511</span>
        </div>
        <div data-aos="fade-up" data-aos-delay="150" class="banner-single">
            <i class="fa fa-3x fa-fax"></i>
            <h2 class="banner-single-title">Fax</h2>
            <span class="banner-single-text">909 South IL Route 83 Suite 104 Elmhurst, IL 60126</span>
        </div>
        <div data-aos="fade-up" data-aos-delay="200" class="banner-single">
            <i class="fa fa-3x fa-at"></i>
            <h2 class="banner-single-title">E-mail</h2>
            <span class="banner-single-text">info@bblogistics.us</span>
        </div>
    </div>

</section>
